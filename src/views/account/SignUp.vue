<template>
    <div class="content">
        ...
        <div class="wrap_btn" style="margin-top:20%">
            <button class="btn_comm" :disabled="!validate()" v-on:click="signUp()">
    ...
    methods: {
        validate() {
            if (this.$utils.isEmpty(this.data.loginId)) {
                return false;
            } else if (this.$utils.isEmpty(this.data.nickName)) {
                return false;
        ...
        async signUp() {
            if (!this.validate()) {
                this.$toast('필수값을 입력해주세요.');
            } else {
                const { data } = await this.$store.dispatch('ACCOUNT_SIGN_UP', this.data);
                localStorage.setItem('loginId', this.data.loginId);
